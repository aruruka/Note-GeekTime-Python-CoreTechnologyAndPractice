import sys
sys.path.append('.')
from quantitative_trading_in_practice.quantitative_trading_4_Pandas_Numpy_Strategies_and_Simulations.utils import read_file, assert_msg, crossover, SMA


class ExchangeAPI:
    def __init__(self, data, cash, commission) -> None:
        assert_msg(0 < cash, "初始现金数量大于0，输入的现金数量: {}".format(cash))
        assert_msg(0 <= commission <= 0.05,
                   "合理的手续费一般不会超过5%，输入的费率：{}".format(commission))
