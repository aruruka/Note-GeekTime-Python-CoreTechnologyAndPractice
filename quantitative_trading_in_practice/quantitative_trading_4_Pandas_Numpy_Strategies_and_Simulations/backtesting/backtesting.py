...
from quantitative_trading_in_practice.quantitative_trading_4_Pandas_Numpy_Strategies_and_Simulations.backtesting.strategy import Strategy
from typing import Type
import numpy as np
import pandas as pd
from numbers import Number
from quantitative_trading_in_practice.quantitative_trading_4_Pandas_Numpy_Strategies_and_Simulations.utils import assert_msg
...
class Backtest:
    ...
    def __init__(self,
        ...
        :param commission:       float                  每次交易手续费率。比如 2% 的手续费的话，此处应为 0.02
        """

        assert_msg(issubclass(strategy_type, Strategy),
                   'strategy_type 不是一个 Strategy 类型')
        assert_msg(issubclass(broker_type, ExchangeAPI),
                   'strategy_type 不是一个Strategy 类型')
